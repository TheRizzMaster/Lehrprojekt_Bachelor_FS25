...
modalForm.addEventListener("submit", async (e) => {
  ...
  const url = `./php/admin_${currentEditType}.php${currentEditId ? `?id=${currentEditId}` : ""}`;
  ...
  data.forEach((mod, idx) => {
    ...
    el.addEventListener("drop", async (e) => {
      ...
      if (draggedItem !== el) {
        ...
        saveOrder(`./php/admin_module.php?action=reorder`, `#modules-${courseId}`);
      }
    });
    list.appendChild(el);
  });
...
async function loadLessons(moduleId) {
  ...
  data.forEach(lesson => {
    ...
    el.addEventListener("drop", async (e) => {
      ...
      if (draggedItem !== el) {
        el.parentNode.insertBefore(draggedItem, el);
        saveOrder(`./php/admin_lesson.php?action=reorder`, `#lessons-${moduleId}`);
      }
    });
    list.appendChild(el);
  });
